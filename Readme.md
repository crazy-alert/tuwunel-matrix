# 🚀 Matrix-сервер на Tuwunel с Docker Compose

Этот репозиторий содержит готовый Docker‑стек для развёртывания собственного Matrix‑сервера на базе **Tuwunel** (лёгкий сервер на Rust), PostgreSQL, Caddy (автоматический HTTPS) и Unbound (кеширующий DNS). Всё это упаковано в удобный `docker-compose.yml` – достаточно склонировать, поправить конфиги и запустить.

---

## 📁 Структура репозитория
```text
├── Caddyfile # конфигурация обратного прокси (Caddy)
├── docker-compose.yml # описание всех сервисов
├── tuwunel.toml # основной конфиг Tuwunel
├── unbound-conf/ # каталог для дополнительных конфигов Unbound
│ └── 99-local.conf # (создаётся вами) настройки кеширования и форвардеров
├── unbound-data/ # данные Unbound (кеш, ключи DNSSEC) – будет создан автоматически
└── README.md # этот файл
```
---
#### Если хотите чтоб торчал не matrix.*** поддомен:

В ```Caddyfile``` – нужно заменить ```matrix.ваш-домен.ru``` на ваш выбранный поддомен в двух местах (для основного домена и для порта 8448).
Пример, если вы выбрали ```chat.пример.ру```:

```text
chat.пример.ру {
reverse_proxy tuwunel:8008
}

chat.пример.ру:8448 {
reverse_proxy tuwunel:8008
}
```
В ```tuwunel.toml``` – параметр server_name должен быть точно таким же, как ваш поддомен (например, ```chat.пример.ру```). Это имя сервера, которое будет использоваться в идентификаторах пользователей (@user:chat.пример.ру).

```toml
server_name = "chat.пример.ру"
```
В DNS – на вашем домене нужно создать A-запись для выбранного поддомена, указывающую на IP вашего сервера.

При регистрации клиентов – пользователи будут вводить этот поддомен как адрес сервера (например, ```https://chat.пример.ру```).

Все остальные настройки (порты, внутренние ссылки) остаются без изменений, так как внутри Docker-сети сервисы общаются по именам контейнеров, а не по доменам. Единственное, что Caddy будет ожидать, что запросы приходят именно на этот поддомен, поэтому важно, чтобы DNS был настроен правильно.

Конечно, поддомен может быть любым — не обязательно matrix. Вы можете выбрать, например, ```chat.ваш-домен.ru```, ```messenger.ваш-домен.ru``` или любой другой. Ниже перечислены все места, где нужно внести изменения.

При первом запуске Docker создаст также каталоги `postgres-data/` и `tuwunel-data/` для хранения данных БД и медиафайлов. **Не удаляйте их**, если не хотите потерять пользователей и историю.

---

## 🔧 Предварительные требования

- Сервер (VPS или выделенный) с **Ubuntu 20.04+ / Debian 11+**, минимум 1 GB RAM (рекомендуется 2 GB).
- Установленные **Docker** и **Docker Compose** (обычно `docker-compose` входит в состав Docker).
- Доменное имя, направленное на IP вашего сервера. Понадобятся два поддомена:
    - `matrix.ваш-домен.ru` – для сервера Matrix
    - (опционально) `element.ваш-домен.ru` – если позже захотите поставить веб‑клиент Element
- Открытые порты в фаерволе: **80**, **443**, **8448** (TCP).

---

## ⚙️ Настройка перед запуском

### 1. Клонируйте репозиторий и перейдите в него

```bash
git clone <url-вашего-репозитория>
cd <имя-папки>
```
### 2. Отредактируйте tuwunel.toml
Это главный конфиг сервера. Обязательно замените:
server_name – ваш домен (например, `matrix.example.ru`).
Параметры подключения к БД: укажите тот же пароль, что и в ```docker-compose.yml```.
Секретный токен регистрации – сгенерируйте его командой openssl rand -hex 32 и вставьте в поле token (можно просто придумаь, не генерировать).

Пример готового tuwunel.toml:
```toml
server_name = "matrix.ваш-домен.ru"

[database]
backend = "postgres"
uri = "postgres://tuwunel_user:ваш_пароль_базы_данных@postgres:5432/tuwunel"

[global]
address = "0.0.0.0:8008"
media_directory = "/var/lib/tuwunel/media"

[registration]
enabled = true
token = "сюда_вставьте_сгенерированный_токен"
```
⚠️ Не забудьте после создания первого администратора отключить регистрацию – закомментировать или удалить секцию [registration].

### 3. Отредактируйте Caddyfile
Укажите ваш домен вместо matrix.ваш-домен.ru:

```text
matrix.ваш-домен.ru {
    reverse_proxy tuwunel:8008
}

matrix.ваш-домен.ru:8448 {
    reverse_proxy tuwunel:8008
}
```
Caddy сам получит SSL‑сертификаты от Lets Encrypt.

### 4. (Опционально) Настройте Unbound
По умолчанию Unbound будет работать как рекурсивный резолвер. Для ускорения работы в российских реалиях можно добавить форвардеры с DNS‑over‑TLS. Создайте файл ```unbound-conf/99-local.conf```:

```conf
server:
    cache-min-ttl: 3600
    cache-max-ttl: 86400
    prefetch: yes
    prefetch-key: yes
    msg-cache-size: 100m
    rrset-cache-size: 200m

forward-zone:
    name: "."
    forward-addr: 1.1.1.1@853#cloudflare-dns.com   # DNS-over-TLS
    forward-addr: 8.8.8.8@853#dns.google
```
Если не хотите использовать внешние форвардеры, оставьте файл пустым – Unbound будет ходить в корневые DNS‑серверы напрямую.

### 5. Установите пароль БД в ```docker-compose.yml```
Найдите в файле ```docker-compose.yml``` переменную окружения ```POSTGRES_PASSWORD``` и замените "ваш_пароль_базы_данных" на реальный пароль (тот же, что в tuwunel.toml).

### 🚀 Запуск сервера
Выполните в каталоге с ```docker-compose.yml```:

```bash
docker-compose up -d
```
Через минуту все контейнеры будут запущены. Проверьте логи:
```bash
docker-compose logs -f
```
Если всё хорошо, вы увидите, что Caddy получил сертификаты, а Tuwunel подключился к БД.

### 👤 Создание первого администратора
1. Установите на телефон Element X (iOS / Android) или любой другой Matrix‑клиент.
2. При входе укажите ваш домашний сервер: ```https://matrix.ваш-домен.ru```
3. Зарегистрируйте нового пользователя:
* Логин и пароль – любые
* Когда клиент перенаправит вас в браузер, введите токен регистрации, который вы указали в ```tuwunel.toml```.
4. После успешной регистрации этот пользователь автоматически станет глобальным администратором.

Теперь можно отключить открытую регистрацию:
* Остановите стек: ```docker-compose down```
* В ```tuwunel.toml``` закомментируйте или удалите секцию ```[registration]```
* Запустите снова: ```docker-compose up -d```

Регистрация новых пользователей теперь возможна только через пригласительные токены, которые вы можете создавать прямо из Matrix‑комнаты администратора (#admins). Например, команда:

```text
!admin token create --usage-limit 1 для_друга
```

создаст одноразовый токен, который можно отправить другу в любом мессенджере.

### 🌐 Порты и безопасность
В данной конфигурации наружу открыты только порты 80, 443 и 8448 – все они обслуживаются Caddy. Остальные сервисы (PostgreSQL, Tuwunel, Unbound) доступны только внутри Docker‑сети ```internal``` и не видны из интернета.
Проверить открытые порты можно командой:
```bash
sudo ss -tulpn | grep LISTEN
```
Должны быть видны только :80, :443 и :8448.

### 📦 Управление стеком
* Остановить: ```docker-compose down```
* Запустить: ```docker-compose up -d```
* Перезапустить конкретный сервис: ```docker-compose restart tuwunel```
* Посмотреть логи: ```docker-compose logs -f [имя_сервиса]```
* Обновить образы: ```docker-compose pull && docker-compose up -d```

### ❓ Часто задаваемые вопросы
#### ❔ Как сгенерировать секретный токен?
Выполните в терминале:
```bash
openssl rand -hex 32
```
Или используйте любой генератор случайных строк.

####❔ Почему Unbound не работает / не резолвит?
Убедитесь, что в ```docker-compose.yml``` у сервисов ```tuwunel``` и ```caddy``` прописана опция ``` dns: 172.20.0.53```. Проверьте, что Unbound запустился:

```bash
docker-compose logs unbound
```
Если нужна отладка, зайдите внутрь контейнера Unbound:

```bash
docker exec -it tuwunel-unbound sh
drill matrix.org @127.0.0.1
```
#### ❔ Как добавить ещё одного администратора?
Первый администратор может выдавать права другим через команду в комнате #admins:
```text
!admin make @пользователь:ваш-домен.ru admin
```
#### ❔ Хочу поставить веб‑клиент Element.
Добавьте в ```docker-compose.yml``` сервис ```element``` и настройте ещё один поддомен в ```Caddy```. Это выходит за рамки данного README, но в интернете много готовых примеров.

#### ❔ Как сделать бэкап?
Остановите стек и скопируйте каталоги ```postgres-data/```, ```tuwunel-data/``` и ```unbound-data/``` в надёжное место.

#### 📚 Полезные ссылки
* [Официальная документация Tuwunel](https://gitlab.com/tuwunel/tuwunel)
* [Caddy Server](https://caddyserver.com/docs/)
* [Unbound](https://unbound.docs.nlnetlabs.com/en/latest/)
